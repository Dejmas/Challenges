...
template <class Vec> 
void show_vec(std::string_view name, const Vec & vec) {
    std::cerr << name << ": ";
    for (const auto & x : vec) std::cerr << x << " ";
    std::cerr << "\n";
}
...
private:
    ...
    std::vector<int> getHighestNodes(int from, int to);
...
std::vector<int> PushRelabel::getHighestNodes(int from, int to) {
...
int main () {
    ...
    while (edges -- > 0) {
        ...
        network.addEdge(from-1, to-1, flow);
    } 
    long flow = network.maxFlow(0, N-1);
    std::cout << flow << "\n";
    return 0;
}
